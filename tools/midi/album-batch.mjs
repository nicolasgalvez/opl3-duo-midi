...
const DURATIONS = (() => { try { return JSON.parse(readFileSync(DURATIONS_FILE, 'utf8')) } catch { return {} } })()
const results = []
const deferred = []
...
function albums() {
  return readdirSync(TOP10, { withFileTypes: true })
    ...
    .map((d) => d.name).sort()
    .map((name) => ({
      name, dir: `${TOP10}/${name}`, out: `${OUTDIR}/${name}.mp4`,
...
function needGB(seconds) { return (seconds / 3600) * PEAK_GB_PER_HR * SAFETY_MULT + SAFETY_FIXED_GB }
...
function purgeObsMkv() {
  ...
  try {
    for (const f of readdirSync(MOVIES)) {
      ...
      const p = `${MOVIES}/${f}`
      try { const sz = statSync(p).size; unlinkSync(p); freed += sz } catch {}
    }
  } catch {}
  if (freed) log(`purged OBS mkv intermediates: ${(freed / 1e9).toFixed(2)}GB`)
}

async function connectObs() { const obs = new OBSWebSocket(); try { await obs.connect(OBS_URL); return obs } catch { return null } }
async function ensureObsUp() {
  let obs = await connectObs(); if (obs) return obs
  ...
  log('OBS still down after 90s'); return null
}
async function ensureIdle(obs) { try { const { outputActive } = await obs.call('GetRecordStatus'); if (outputActive) { log('OBS was recording — stopping'); await obs.call('StopRecord'); await sleep(1500) } } catch {} }
...
async function waitForNoOtherRender() {
  ...
  while (otherRenderRunning() && Date.now() - start < INFLIGHT_MAX_MS) { await sleep(30000); if (++n % 10 === 0) log(`…waiting on in-flight render (${Math.round((Date.now() - start) / 60000)} min)`) }
...
function fullReset() {
  try {
    const outs = easymidi.getOutputs(); const name = outs.find((n) => n.includes('OPL3Duo')) || outs[0]
    if (!name) { log('reset: no MIDI output found'); return }
    const out = new easymidi.Output(name)
    for (let ch = 0; ch < 16; ch++) {
      out.send('cc', { controller: 120, value: 0, channel: ch }); out.send('cc', { controller: 123, value: 0, channel: ch })
      out.send('cc', { controller: 121, value: 0, channel: ch }); out.send('cc', { controller: 64, value: 0, channel: ch })
      out.send('cc', { controller: 1, value: 0, channel: ch }); out.send('cc', { controller: 11, value: 127, channel: ch })
      out.send('pitch', { value: 8192, channel: ch }); out.send('program', { number: 0, channel: ch })
    }
    out.send('reset')
...
function loudness(file) {
  try {
    ...
    const mean = out.match(/mean_volume:\s*(-?[\d.]+) dB/); const max = out.match(/max_volume:\s*(-?[\d.]+) dB/)
...
function render(albumDir, outputAbs, title, seconds) {
  return new Promise((resolve) => {
    const args = ['render', albumDir, '--album', '--obs', '--layout', 'minimized',
      '--audio-device', AUDIO_DEVICE, '--obs-source', OBS_SOURCE, '--title', title, '-o', outputAbs]
    const child = spawn(OPL, args, { cwd: MIDI_CWD })
    let killed = false, totalSec = seconds || null
    const budget0 = (seconds ? seconds * 1000 * 1.4 + 900000 : 0)
    let timer = setTimeout(() => fail('timeout-startup'), 300000)
    if (budget0) { clearTimeout(timer); timer = setTimeout(() => fail('timeout'), budget0) }
    function fail(reason) { if (killed) return; killed = true; log(`  killing render (${reason})`); try { child.kill('SIGKILL') } catch {}; resolve({ ok: false, error: reason, totalSec }) }
    function onData(buf) {
      const s = buf.toString(); const m = s.match(/([\d.]+)s total/) || s.match(/\(([\d.]+)s\)/)
      if (m && !seconds && !totalSec) { totalSec = parseFloat(m[1]); clearTimeout(timer); timer = setTimeout(() => fail('timeout'), totalSec * 1000 * 1.4 + 900000) }
    }
    child.stdout.on('data', onData); child.stderr.on('data', onData)
    child.on('close', (code) => { if (killed) return; clearTimeout(timer); resolve({ ok: code === 0, code, totalSec }) })
    child.on('error', (e) => { if (killed) return; clearTimeout(timer); resolve({ ok: false, error: e.message, totalSec }) })
  })
}

;(async () => {
  const list = albums()
  log(`=== ALBUM batch start: ${list.length} albums, ${(list.reduce((s, a) => s + a.seconds, 0) / 3600).toFixed(1)}h music -> ${OUTDIR} ===`)
  let obs = null
  for (const a of list) {
    ...
    if (existsSync(a.out) && statSync(a.out).size > MIN_OK_BYTES) {
      rec.ok = true; rec.skipped = true; rec.sizeMB = +(statSync(a.out).size / 1e6).toFixed(1)
      results.push(rec); writeFileSync(PROGRESS, JSON.stringify({ results, deferred }, null, 2))
      log(`SKIP (exists) ${a.title} (${rec.sizeMB}MB)`); continue
    }
    ...
    if (existsSync(a.out) && statSync(a.out).size > MIN_OK_BYTES) {
      rec.ok = true; rec.skipped = true; rec.sizeMB = +(statSync(a.out).size / 1e6).toFixed(1)
      results.push(rec); writeFileSync(PROGRESS, JSON.stringify({ results, deferred }, null, 2))
      log(`SKIP (finished by standalone render) ${a.title} (${rec.sizeMB}MB)`); continue
    }
    if (a.tracks === 0) { rec.ok = false; rec.error = 'empty-folder'; results.push(rec); writeFileSync(PROGRESS, JSON.stringify({ results, deferred }, null, 2)); log(`FAIL empty ${a.title}`); continue }

    // ---- disk gate ----
    const free = freeGB('/'), need = needGB(a.seconds)
    if (free < need || free < HARD_FLOOR_GB) {
      rec.deferred = true; rec.reason = `disk: free ${free.toFixed(1)}GB < need ${need.toFixed(1)}GB`
      deferred.push(a.title); results.push(rec); writeFileSync(PROGRESS, JSON.stringify({ results, deferred }, null, 2))
      log(`DEFER ${a.title} (${rec.minutes}min) — ${rec.reason}; trying smaller albums`)
      continue
    }

    log(`RENDER ${a.title} (${a.tracks} tracks, ${rec.minutes}min) — free ${free.toFixed(1)}GB, need ${need.toFixed(1)}GB`)
    obs = await ensureObsUp(); if (obs) await ensureIdle(obs)
    const start = Date.now()
    const res = await render(a.dir, a.out, a.title, a.seconds)
    if (obs) { try { await ensureIdle(obs) } catch { obs = null } }
    await fullReset(); await sleep(2000)
    ...
    rec.ok = !!okFile; rec.wallSec = Math.round((Date.now() - start) / 1000)
    if (okFile) { const l = loudness(a.out); rec.meanDb = l.meanDb; rec.maxDb = l.maxDb; rec.sizeMB = +(statSync(a.out).size / 1e6).toFixed(1) }
    else rec.error = res.error || `exit ${res.code}`
    results.push(rec); writeFileSync(PROGRESS, JSON.stringify({ results, deferred }, null, 2))
    log(`${okFile ? 'OK' : 'FAIL'} ${a.title} ${okFile ? `(${rec.sizeMB}MB, mean ${rec.meanDb}dB, ${rec.wallSec}s)` : `(${rec.error})`} — free now ${freeGB('/').toFixed(1)}GB`)
  }
  const ok = results.filter((r) => r.ok).length
  log(`=== ALBUM batch DONE: ${ok} rendered, ${deferred.length} deferred${deferred.length ? ` [${deferred.join(', ')}]` : ''} ===`)
