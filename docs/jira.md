...
- **Key:** `ODM`
- **Board:** https://procyoncreative.atlassian.net/jira/software/c/projects/ODM/boards
...
- **No work without a ticket.** Every branch must reference an `ODM-NNN` Jira ticket. If one doesn't exist, create it first via `/jira-ticket create` (or in the Jira UI) before opening the branch.
- **Branch name format:** `ODM-NNN-short-description` (e.g. `ODM-42-add-login-form`).
- **Ticket requirements:**
